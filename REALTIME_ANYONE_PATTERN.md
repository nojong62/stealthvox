...
브랜치 `realtime-secure-webrtc` · 기준 커밋 `c0bc435c` · 최종 실기기 검증 2026-07-28 (Samsung SM-S931N)
...
대신 **첫 발화만 `gpt-4o-transcribe`, 이후 `gpt-4o-mini-transcribe`**로
간다. 첫 문장이 상대와 상황을 정하므로 거기만 비용을 더 쓴다.
...
| 마이크 활성화 | 진입 +0.19초 |
| 발화 종료 → 전사 확정 | 0.43초 |
| 발화 종료 → 유저 영어 음성 시작 | 2.10초 |
| 발화 종료 → 유저 음성 재생 완료 | 5.40초 |
| 발화 종료 → AI 첫 음성 | 8.15초 |

다른 모드를 붙일 때 이 값과 비교하면 어디가 새는지 바로 보인다.
...
4. 동음 오인식 (4.7). 첫 발화 정밀 모델의 효과는 **아직 실기기 미검증**이다.
